...
- Project: https://iworld-bench.com/
- Code: https://github.com/EmbodiedCity/iWorld-Bench
- Data / benchmark: https://huggingface.co/datasets/EmbodiedCity/iWorld-Bench-Dataset
- Demo / video: https://iworld-bench.com/
- Official figures: https://iworld-bench.com/

## TL;DR

iWorld-Bench: A Benchmark for Interactive World Models with a Unified Action Generation Framework is included because it improves the repository's coverage of evaluation infrastructure for interactive embodied generation rather than adding yet another method paper with unclear downstream value. The key contribution is turns interactive world-model evaluation into a multi-input, action-space-unified benchmark instead of model-specific camera-control demos. For this repository, the most important point is that the paper gives a concrete benchmark, protocol, or release surface that can be reused when we need to judge whether world models or VLA systems are actionable, reliable, controllable, and reproducible. Main caveat: The current release is centered on interactive video-generation style world models rather than manipulation-policy world models, so mapping to robotics execution still requires care.

## Novelty

- What is actually new: turns interactive world-model evaluation into a multi-input, action-space-unified benchmark instead of model-specific camera-control demos
...
1. Defines a benchmark/problem setting centered on: train and test interactive world models on distance perception, action control, camera control, trajectory following, and memory across diverse scenes.
2. Provides a reusable evaluation method built around: unified action-generation framework that standardizes interaction control across modalities and scores world models on visual generation, trajectory following, and memory with nine metrics.
...
- Dataset / benchmark: 330k video clips, 2.1k selected high-quality samples, and 4.9k test tasks spanning varied perspectives, weather, and scenes
...
- Core pipeline: unified action-generation framework that standardizes interaction control across modalities and scores world models on visual generation, trajectory following, and memory with nine metrics
...
- Source: https://iworld-bench.com/
- Render:
  Official figure/demo page: https://iworld-bench.com/
...
|---|---|---|---|
| Project reports 330K video clips, 2.1K selected samples, and 4.9K test tasks. | https://iworld-bench.com/ | verified | Official project page exposes these counts. |
| Benchmark evaluates interactive world models across multiple inputs, action control, camera control, memory, and all-weather settings. | https://iworld-bench.com/ | verified | Comparison table on official project page states the capability coverage. |
| Official code and dataset are public. | https://github.com/EmbodiedCity/iWorld-Bench | verified | GitHub repository and dataset links are public. |
...
- Method limitations: The current release is centered on interactive video-generation style world models rather than manipulation-policy world models, so mapping to robotics execution still requires care.
...
- Relevance to interactive embodied generation: High-value benchmark for first-person interactive world-model evaluation, camera control, memory, and multi-scene action responsiveness.
...
- Code / checkpoint availability: https://github.com/EmbodiedCity/iWorld-Bench.
