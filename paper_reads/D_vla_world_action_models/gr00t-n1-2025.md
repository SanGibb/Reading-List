...
- Code: https://huggingface.co/nvidia/GR00T-N1-2B
- Data / benchmark: https://research.nvidia.com/labs/lpr/publication/gr00tn1_2025/
- Demo / video: https://research.nvidia.com/labs/lpr/publication/gr00tn1_2025/
- Official figures: https://research.nvidia.com/labs/lpr/publication/gr00tn1_2025/

## TL;DR

GR00T N1: An Open Foundation Model for Generalist Humanoid Robots is included because it addresses **generalist humanoid robot control under language and visual instructions** with a method centered on **open foundation model for humanoid robots with reasoning and action components trained on diverse robot data**. For our repository, the important point is not venue alone but that the work gives concrete evidence for vision-language-action models, robot foundation policies, and action-conditioned world models. Main caveat: Humanoid focus may not transfer directly to tabletop generation tasks; later GR00T releases should be tracked separately.

## Novelty

- What is actually new: one of the first open humanoid foundation models with official weights and reproducible release path.
...
1. Defines a generalist humanoid robot control under language and visual instructions problem setting that is directly relevant to interactive embodied generation.
2. Introduces the core method: open foundation model for humanoid robots with reasoning and action components trained on diverse robot data.
3. Provides evidence through large mixed humanoid/robot datasets plus simulation benchmark evaluations and released model artifacts and makes the paper useful for our VLA and World-Action Models branch.
...
- Output: generalist humanoid robot control under language and visual instructions outputs, represented as generated worlds, executable assets, spatial answers, policy actions, or benchmark scores depending on branch.
...
- Dataset / benchmark: large mixed humanoid/robot datasets plus simulation benchmark evaluations and released model artifacts.
...
- Core pipeline: open foundation model for humanoid robots with reasoning and action components trained on diverse robot data.
...
- Render:
  Official figure/demo page: https://research.nvidia.com/labs/lpr/publication/gr00tn1_2025/
...
|---|---|---|---|
| Primary source exists and is 2024+. | https://arxiv.org/abs/2503.14734 | verified | Year recorded as 2025. |
| Dataset/method/task fields are identifiable. | https://arxiv.org/abs/2503.14734 | verified | Registry fields were extracted from primary or official descriptions. |
| Demo or visual quality decision. | https://research.nvidia.com/labs/lpr/publication/gr00tn1_2025/ | verified | decision=`strong`, score=4. |
| Project relevance. | local taxonomy and harness | verified | Important VLA baseline for humanoid embodiments and synthetic-data-to-policy evaluation. |

## Evidence

- Main metrics: Official NVIDIA page and arXiv describe an open humanoid foundation model with simulation benchmarks and multi-embodiment training evidence.
...
- Method limitations: Humanoid focus may not transfer directly to tabletop generation tasks; later GR00T releases should be tracked separately.
...
- Relevance to interactive embodied generation: Important VLA baseline for humanoid embodiments and synthetic-data-to-policy evaluation.
...
- Code / checkpoint availability: https://huggingface.co/nvidia/GR00T-N1-2B.
