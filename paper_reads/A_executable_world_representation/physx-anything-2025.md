# PhysX-Anything: Simulation-Ready Physical 3D Assets from Single Image

candidate_id: CAND-0016
branch: A
decision: accepted_for_registry
authors: Cao et al.
year: 2025
venue: arXiv

## Source Links

- Paper: https://arxiv.org/abs/2511.13648
- Project: https://physx-anything.github.io/
- Code: https://physx-anything.github.io/
- Data / benchmark: https://physx-anything.github.io/
- Demo / video: https://physx-anything.github.io/
- Official figures: https://physx-anything.github.io/

## TL;DR

PhysX-Anything: Simulation-Ready Physical 3D Assets from Single Image is included because it addresses **generate simulation-ready physical 3D assets from a single image** with a method centered on **VLM-based physical 3D generative model with efficient geometry tokenization and explicit geometry, articulation, and physical attributes**. For our repository, the important point is not venue alone but that the work gives concrete evidence for executable world structures, object parts, affordances, task predicates, and physical/deformable state. Main caveat: Object-level asset generation; full scene/task integration still requires composition and validation.

## Novelty

- What is actually new: turns image-conditioned 3D generation into sim-ready assets with physical and articulation metadata.
- Difference from prior work: it adds executable, interactive, physical, spatial, or policy-facing structure rather than stopping at static visual quality.
- Why the delta matters: our target task needs generated or reconstructed worlds that agents can reason about, validate, or act in, so this paper contributes reusable structure rather than only a visual sample gallery.

## Contributions

1. Defines a generate simulation-ready physical 3D assets from a single image problem setting that is directly relevant to interactive embodied generation.
2. Introduces the core method: VLM-based physical 3D generative model with efficient geometry tokenization and explicit geometry, articulation, and physical attributes.
3. Provides evidence through PhysX-Mobility with 2K+ common objects plus in-the-wild single-image inputs and physical/articulation annotations and makes the paper useful for our Executable World Representation branch.

## Task

- Input: paper-specific observations, prompts, scene assets, robot observations, or benchmark inputs described by the original source.
- Output: generate simulation-ready physical 3D assets from a single image outputs, represented as generated worlds, executable assets, spatial answers, policy actions, or benchmark scores depending on branch.
- Setting: Executable World Representation within an interactive embodied generation reading list.
- Success criterion: strong source evidence, branch fit, usable data/method description, and visual/demo quality of `strong` when the work depends on generation or robot execution.

## Data

- Dataset / benchmark: PhysX-Mobility with 2K+ common objects plus in-the-wild single-image inputs and physical/articulation annotations.
- Scale: use the scale reported by the source; exact split details should be checked in the paper before citation.
- Modalities: 3D scenes/assets, images/videos, depth/multi-view observations, robot trajectories, actions, or benchmark annotations depending on the paper.
- Collection / annotation: source-specific; this run records only primary-source claims and does not independently audit annotation pipelines.
- Splits / evaluation protocol: follow the paper/project page; local reproduction was not performed.

## Method

- Core pipeline: VLM-based physical 3D generative model with efficient geometry tokenization and explicit geometry, articulation, and physical attributes.
- Model / representation: the useful abstraction for this repository is the mapping from source input to executable world representation, spatial state, robot action, or evaluation signal.
- Training or optimization: use the paper-specific training, optimization, search, or benchmark construction described by the primary source.
- Inference / deployment: relevant deployment mode is generated-scene inspection, simulator import, robot rollout, spatial VLM evaluation, or world-model benchmark execution.
- Losses or metrics: use reported metrics only as source-attributed evidence until reproduced locally.

## Key Figures / Architecture

figure_status: linked_official

- Figure / demo: official project, paper, or architecture material.
- Source: https://physx-anything.github.io/
- Render:
  Official figure/demo page: https://physx-anything.github.io/
- What it shows: the official figure/demo evidence for the method pipeline, generated results, robot execution, or benchmark design.
- Why it matters: it is the visual anchor used by Quality Reviewer; no local reproduction was performed in this initialization run.

## Evidence Trail

| Claim | Source | Status | Notes |
|---|---|---|---|
| Primary source exists and is 2024+. | https://arxiv.org/abs/2511.13648 | verified | Year recorded as 2025. |
| Dataset/method/task fields are identifiable. | https://arxiv.org/abs/2511.13648 | verified | Registry fields were extracted from primary or official descriptions. |
| Demo or visual quality decision. | https://physx-anything.github.io/ | verified | decision=`strong`, score=4. |
| Project relevance. | local taxonomy and harness | verified | Strong object-level source for PhysicalProperty, KinematicJoint, AssetToSimulator, and contact-rich policy-learning validation. |

## Evidence

- Main metrics: The paper reports sim-ready physical assets from single images, explicit geometry/articulation/physical properties, PhysX-Mobility with 2K+ annotated objects, and simulation-based policy learning evidence.
- Qualitative results: visual/demo decision is `strong` with score 4; generation-heavy entries need adequate or strong evidence.
- Ablations: not independently reproduced; use original paper/project ablations when citing.
- Baselines: compare against the paper's own baseline list before making SOTA claims.
- Reproducibility signals: source quality is `primary_or_official`, demo score is 4, evidence strength is 4.

## Limitations

- Method limitations: Object-level asset generation; full scene/task integration still requires composition and validation.
- Experimental limitations: this initialization run did not train models, download large datasets, or reproduce metrics.
- Demo / visual limitations: official examples may be curated; local reproduction or independent videos are needed before strong deployment claims.
- Claims that remain unverified: exact code/checkpoint maturity, license constraints, and benchmark leaderboard status may change after 2026-06-06.

## Project Relevance

- Relevance to interactive embodied generation: Strong object-level source for PhysicalProperty, KinematicJoint, AssetToSimulator, and contact-rich policy-learning validation.
- Reusable fields: Scene, Object, Part, Affordance, PhysicalProperty, SpatialRelation, StateTransition, Action, Policy, BenchmarkMetric, and ValidationReport as applicable.
- Possible baseline role: use this paper as a branch-specific baseline or validator for Executable World Representation.
- Implications for our task / benchmark: it helps decide whether generated scenes are only visually plausible or actually useful for interactive, spatial, physical, or policy-facing embodied tasks.

## Reproduction / Follow-up

- What to check before using: official code/data release status, license, exact benchmark split, and whether demos can be run locally.
- Code / checkpoint availability: https://physx-anything.github.io/.
- Citation or related-work caveats: phrase strong claims as reported by the source unless we independently reproduce them.
